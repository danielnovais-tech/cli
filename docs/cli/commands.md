...
Blackbox Code supports several built-in commands to help you manage your session, customize the interface, and control its behavior. These commands are prefixed with a forward slash (`/`), an at symbol (`@`), or an exclamation mark (`!`).
...
- **`/bug`**
  - **Description:** File an issue about Blackbox Code. By default, the issue is filed within the GitHub repository for Blackbox Code. The string you enter after `/bug` will become the headline for the bug being filed. The default `/bug` behavior can be modified using the `bugCommand` setting in your `.blackboxcli/settings.json` files.
  ...
  - **Sub-commands:**
    - **`save`**
      ...
      - **Details on Checkpoint Location:** The default locations for saved chat checkpoints are:
        - Linux/macOS: `~/.blackboxcli/tmp/<project_hash>/`
        - Windows: `C:\Users\<YourUsername>\.blackboxcli\tmp\<project_hash>\`
...
- **`/summary`**
  - **Description:** Generate a comprehensive project summary from the current conversation history and save it to `.blackboxcli/PROJECT_SUMMARY.md`. This summary includes the overall goal, key knowledge, recent actions, and current plan, making it perfect for resuming work in future sessions.
  - **Usage:** `/summary`
  - **Features:**
    ...
    - Automatically saves to `.blackboxcli/PROJECT_SUMMARY.md` in your project root
...
- **`/copy`**
  - **Description:** Copies the last output produced by Blackbox Code to your clipboard, for easy sharing or reuse.
...
- **`/extensions`**
  - **Description:** Lists all active extensions in the current Blackbox Code session. See [Blackbox Code Extensions](../extension.md).

- **`/help`** (or **`/?`**)
  - **Description:** Display help information about the Blackbox Code, including available commands and their usage.
...
- **`/memory`**
  - **Description:** Manage the AI's instructional context (hierarchical memory loaded from `BLACKBOX.md` files by default; configurable via `contextFileName`).
  - **Sub-commands:**
    - **`add`**:
      - **Description:** Adds the following text to the AI's memory. Usage: `/memory add <text to remember>`
    - **`show`**:
      - **Description:** Display the full, concatenated content of the current hierarchical memory that has been loaded from all context files (e.g., `BLACKBOX.md`). This lets you inspect the instructional context being provided to the model.
    - **`refresh`**:
      - **Description:** Reload the hierarchical instructional memory from all context files (default: `BLACKBOX.md`) found in the configured locations (global, project/ancestors, and sub-directories). This updates the model with the latest context content.
...
- **`/settings`**
  - **Description:** Open the settings editor to view and modify Blackbox Code settings.
  - **Details:** This command provides a user-friendly interface for changing settings that control the behavior and appearance of Blackbox Code. It is equivalent to manually editing the `.blackboxcli/settings.json` file, but with validation and guidance to prevent errors.
...
- **`/stats`**
  - **Description:** Display detailed statistics for the current Blackbox Code session, including token usage, cached token savings (when available), and session duration. Note: Cached token information is only displayed when cached tokens are being used, which occurs with API key authentication but not with OAuth authentication at this time.

- [**`/theme`**](./themes.md)
  - **Description:** Open a dialog that lets you change the visual theme of Blackbox Code.
...
- **`/agents`**
  ...
  - **Sub-commands:**
    ...
    - **`manage`**:
      ...
      - **Usage:** `/agents manage`
  - **Storage Locations:**
    - **Project-level:** `.blackboxcli/agents/` (shared with team, takes precedence)
...
- **`/quit-confirm`**
  - **Description:** Show a confirmation dialog before exiting Blackbox Code, allowing you to choose how to handle your current session.
  - **Usage:** `/quit-confirm`
  - **Features:**
...
- **`/quit`** (or **`/exit`**)
  - **Description:** Exit Blackbox Code immediately without any confirmation dialog.
...
- **`/vim`**
  ...
  - **Features:**
    ...
    - **Persistent setting:** Vim mode preference is saved to `~/.blackboxcli/settings.json` and restored between sessions
  - **Status indicator:** When enabled, shows `[NORMAL]` or `[INSERT]` in the footer

- **`/init`**
  - **Description:** Analyzes the current directory and creates a `BLACKBOX.md` context file by default (or the filename specified by `contextFileName`). If a non-empty file already exists, no changes are made. The command seeds an empty file and prompts the model to populate it with project-specific instructions.

### Custom Commands

For a quick start, see the [example](#example-a-pure-function-refactoring-command) below.

Custom commands allow you to save and reuse your favorite or most frequently used prompts as personal shortcuts within Blackbox Code. You can create commands that are specific to a single project or commands that are available globally across all your projects, streamlining your workflow and ensuring consistency.
...
- A file at `~/.blackboxcli/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.blackboxcli/commands/git/commit.toml` becomes the namespaced command `/git:commit`.
...
```toml
# In: <project>/.blackboxcli/commands/changelog.toml
...
When a custom command attempts to execute a shell command, Blackbox Code will now prompt you for confirmation before proceeding. This is a security measure to ensure that only intended commands can be run.
...
````toml
# In: <project>/.blackboxcli/commands/git/commit.toml
...
```bash
mkdir -p ~/.blackboxcli/commands/refactor
touch ~/.blackboxcli/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.blackboxcli/commands/refactor/pure.toml` in your editor and add the following content. We are including the optional `description` for best practice.

```toml
# In: ~/.blackboxcli/commands/refactor/pure.toml
...
Blackbox Code will then execute the multi-line prompt defined in your TOML file.
...
The `!` prefix lets you interact with your system's shell directly from within Blackbox Code.
...
- **`!<shell_command>`**
  ...
  - **Examples:**
    - `!ls -la` (executes `ls -la` and returns to Blackbox Code)
    - `!git status` (executes `git status` and returns to Blackbox Code)
...
- **`!` (Toggle shell mode)**
  - **Description:** Typing `!` on its own toggles shell mode.
    ...
    - **Exiting shell mode:**
      - When exited, the UI reverts to its standard appearance and normal Blackbox Code behavior resumes.

- **Caution for all `!` usage:** Commands you execute in shell mode have the same permissions and impact as if you ran them directly in your terminal.

- **Environment Variable:** When a command is executed via `!` or in shell mode, the `BLACKBOX_CODE=1` environment variable is set in the subprocess's environment. This allows scripts or tools to detect if they are being run from within the CLI.
