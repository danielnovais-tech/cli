# Blackbox Code Configuration

Blackbox Code offers several ways to configure its behavior, including environment variables, command-line arguments, and settings files. This document outlines the different configuration methods and available settings.
...
Blackbox Code uses JSON settings files for persistent configuration. There are four locations for these files:

- **System defaults file:**
  - **Location:** `/etc/blackbox-cli/system-defaults.json` (Linux), `C:\ProgramData\blackbox-cli\system-defaults.json` (Windows) or `/Library/Application Support/BlackboxCode/system-defaults.json` (macOS). The path can be overridden using the `BLACKBOX_CODE_SYSTEM_DEFAULTS_PATH` environment variable.
...
- **User settings file:**
  - **Location:** `~/.blackboxcli/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Blackbox Code sessions for the current user.
- **Project settings file:**
  - **Location:** `.blackboxcli/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Blackbox Code from that specific project. Project settings override user settings.

- **System settings file:**
  - **Location:** `/etc/blackbox-cli/settings.json` (Linux), `C:\ProgramData\blackbox-cli\settings.json` (Windows) or `/Library/Application Support/BlackboxCode/settings.json` (macOS). The path can be overridden using the `BLACKBOX_CODE_SYSTEM_SETTINGS_PATH` environment variable.
  - **Scope:** Applies to all Blackbox Code sessions on the system, for all users. System settings override user and project settings. May be useful for system administrators at enterprises to have controls over users' Blackbox Code setups.

**Note on environment variables in settings:** String values within your `settings.json` files can reference environment variables using either `$VAR_NAME` or `${VAR_NAME}` syntax. These variables will be automatically resolved when the settings are loaded. For example, if you have an environment variable `MY_API_TOKEN`, you could use it in `settings.json` like this: `"apiKey": "$MY_API_TOKEN"`.

### The `.blackboxcli` directory in your project

In addition to a project settings file, a project's `.blackboxcli` directory can contain other project-specific files related to Blackbox Code's operation, such as:
...
- **`bugCommand`** (object):
  ...
  - **Default:** `"urlTemplate": "https://github.com/llmcod/blackbox_cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](./themes.md) for Blackbox Code.
  - **Default:** `"Default"`
...
- **`sandbox`** (boolean or string):
  - **Description:** Controls whether and how to use sandboxing for tool execution. If set to `true`, Blackbox Code uses a pre-built `blackbox-cli-sandbox` Docker image. For more information, see [Sandboxing](#sandboxing).
  - **Default:** `false`
...
- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context Protocol (MCP) servers for discovering and using custom tools. Blackbox Code attempts to connect to each configured MCP server to discover available tools. If multiple MCP servers expose a tool with the same name, the tool names will be prefixed with the server alias you defined in the configuration (e.g., `serverAlias__actualToolName`) to avoid conflicts. Note that the system might strip certain schema properties from MCP tool definitions for compatibility. At least one of `command`, `url`, or `httpUrl` must be provided. If multiple are specified, the order of precedence is `httpUrl`, then `url`, then `command`.
  - **Default:** Empty
  - **Properties:**
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for Blackbox Code. For more information, see [Telemetry](../telemetry.md).
...
- **Location:** `~/.blackboxcli/tmp/<project_hash>/shell_history`
...
- **`OPENAI_MODEL`**:
  ...
  - Example: `export OPENAI_MODEL="blackbox3-coder-plus"`
- **`GEMINI_SANDBOX`**:
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create a file named `sandbox-macos-<profile_name>.sb` in your project's `.blackboxcli/` directory (e.g., `my-project/.blackboxcli/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env` files by default to prevent interference with the CLI behavior. Use `.blackboxcli/.env` files if you need to set these for Blackbox Code specifically.
- **`NO_COLOR`**:
...
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the Blackbox model to use for this session.
  - Example: `npm start -- --model blackbox3-coder-plus`
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes Blackbox Code in a non-interactive mode.
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  - Starts an interactive session with the provided prompt as the initial input.
  - The prompt is processed within the interactive session, not before it.
  - Cannot be used when piping input from stdin.
  - Example: `blackbox -i "explain this code"`
- **`--sandbox`** (**`-s`**):
...
- **`--approval-mode <mode>`**:
  ...
  - Example: `blackbox --approval-mode auto-edit`
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation dialog.
  - Example: `blackbox --allowed-tools "ShellTool(git status)"`
- **`--telemetry`**:
...
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  ...
  - Use the special term `blackbox -e none` to disable all extensions.
  - Example: `blackbox -e my-extension -e my-other-extension`
- **`--list-extensions`** (**`-l`**):
...
- **`--tavily-api-key <api_key>`**:
  ...
  - Example: `blackbox --tavily-api-key tvly-your-api-key-here`
...
### Example Context File Content (e.g., `BLACKBOX.md`)
...
Blackbox Code can execute potentially unsafe operations (like shell commands and file modifications) within a sandboxed environment to protect your system.
...
By default, it uses a pre-built `blackbox-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at `.blackboxcli/sandbox.Dockerfile` in your project's root directory. This Dockerfile can be based on the base sandbox image:
...
When `.blackboxcli/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment variable when running Blackbox Code to automatically build the custom sandbox image:

```bash
BUILD_SANDBOX=1 blackbox -s
```

## Usage Statistics

To help us improve Blackbox Code, we collect anonymized usage statistics. This data helps us understand how the CLI is used, identify common issues, and prioritize new features.
...
- **`enableWelcomeBack`** (boolean):
  ...
  - **Details:** When enabled, Blackbox Code will automatically detect if you're returning to a project with a previously generated project summary (`.blackboxcli/PROJECT_SUMMARY.md`) and show a dialog allowing you to continue your previous conversation or start fresh. This feature integrates with the `/chat summary` command and quit confirmation dialog. See the [Welcome Back documentation](./welcome-back.md) for more details.
