...
Subagents are specialized AI assistants that handle specific types of tasks within Blackbox Code. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
- **Project-level**: `.blackboxcli/agents/` (takes precedence)
- **User-level**: `~/.blackboxcli/agents/` (fallback)
...
Blackbox Code proactively delegates tasks based on:
