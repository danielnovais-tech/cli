...
You can enable checkpointing for the current session by using the `--checkpointing` flag when starting Blackbox Code:

```bash
blackbox --checkpointing
```
