# Blackbox Code Extensions

Blackbox Code supports extensions that can be used to configure and extend its functionality.
...
Within each location, individual extensions exist as a directory that contains a `blackbox-extension.json` file. For example:

`<workspace>/.blackboxcli/extensions/my-extension/blackbox-extension.json`

### `blackbox-extension.json`

The `blackbox-extension.json` file contains the configuration for the extension. The file has the following structure:
...
- `contextFileName`: The name of the file that contains the context for the extension. This will be used to load the context from the workspace. If this property is not used but a `BLACKBOX.md` file is present in your extension directory, then that file will be loaded.
- `excludeTools`: An array of tool names to exclude from the model. You can also specify command-specific restrictions for tools that support it, like the `run_shell_command` tool. For example, `"excludeTools": ["run_shell_command(rm -rf)"]` will block the `rm -rf` command.

When Blackbox Code starts, it loads all the extensions and merges their configurations. If there are any conflicts, the workspace configuration takes precedence.
...
`blackbox extensions install <source> | [options]`
