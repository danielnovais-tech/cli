# MCP servers with Blackbox Code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with Blackbox Code.
...
Blackbox Code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the Blackbox API
...
Blackbox Code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.

### Configure the MCP server in settings.json

You can configure MCP servers at the global level in the `~/.blackboxcli/settings.json` file or in your project's root directory, create or open the `.blackboxcli/settings.json` file. Within the file, add the `mcpServers` configuration block.
...
Blackbox Code supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.blackboxcli/mcp-oauth-tokens.json`
...
When Blackbox Code starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet Blackbox API requirements:
...
Once discovered, MCP tools are available to the Blackbox model like built-in tools. The model will automatically:
...
- **Property stripping:** The system automatically removes certain schema properties (`$schema`, `additionalProperties`) for Blackbox API compatibility
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within Blackbox Code. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Blackbox Code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Blackbox Code" reverent
```
...
## Managing MCP Servers with `blackbox mcp`
...
### Adding a Server (`blackbox mcp add`)

The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.blackboxcli/settings.json` or the project config `.blackboxcli/settings.json` file.

**Command:**

```bash
blackbox mcp add [options] <name> [commandOrUrl] [args...]
```

- `<name>`: A unique name for the server or a preset name (e.g., "remote-code").
- `<commandOrUrl>`: The command to execute (for `stdio`) or the URL (for `http`/`sse`). Optional for preset names.
...
Blackbox CLI includes preset configurations for popular MCP servers that automatically configure the server with the correct URL and authentication headers.

**Available Presets:**

- **`remote-code`**: Blackbox Remote Code MCP Server
  - Requires: `BLACKBOX_API_KEY` environment variable
  - Automatically configures SSE transport with proper authentication

**Example: Adding the remote-code preset**

```bash
# Set your API key in environment
export BLACKBOX_API_KEY="your-api-key-here"

# Add the preset (no URL or transport needed!)
blackbox mcp add remote-code

# Or add to user settings
blackbox mcp add remote-code --scope user
```
...
You can still override preset defaults with custom options:

```bash
blackbox mcp add remote-code --description "My custom description" --timeout 60000
```
...
```bash
# Basic syntax
blackbox mcp add <name> <command> [args...]

# Example: Adding a local server
blackbox mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
blackbox mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
blackbox mcp add --transport http <name> <url>

# Example: Adding an HTTP server
blackbox mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
blackbox mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
blackbox mcp add --transport sse <name> <url>

# Example: Adding an SSE server
blackbox mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
blackbox mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"
```

### Listing Servers (`blackbox mcp list`)
...
### Removing a Server (`blackbox mcp remove`)
...
```bash
blackbox mcp remove <name>
```

**Example:**

```bash
blackbox mcp remove my-server
```
