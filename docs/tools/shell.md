...
This document describes the `run_shell_command` tool for Blackbox Code.
...
## How to use `run_shell_command` with Blackbox Code
...
When `run_shell_command` executes a command, it sets the `BLACKBOX_CODE=1` environment variable in the subprocess's environment. This allows scripts or tools to detect if they are being run from within the CLI.
