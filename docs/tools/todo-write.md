...
This document describes the `todo_write` tool for Blackbox Code.
...
## How to use `todo_write` with Blackbox Code
...
The tool stores todo lists in your home directory (`~/.blackboxcli/todos/`) with session-specific files, so each coding session maintains its own task list.
