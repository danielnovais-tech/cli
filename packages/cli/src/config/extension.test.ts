...
const EXTENSIONS_DIRECTORY_NAME = path.join('.blackboxcli', 'extensions');
...
describe('loadExtensions', () => {
  ...
  beforeEach(() => {
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'blackbox-cli-test-workspace-'),
    );
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'blackbox-cli-test-home-'),
    );
  ...
  it('should load context file path when BLACKBOX.md is present', () => {
    const workspaceExtensionsDir = path.join(
      tempWorkspaceDir,
      EXTENSIONS_DIRECTORY_NAME,
    );
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'BLACKBOX.md'),
    ]);
  ...
  it('should filter out disabled extensions', () => {
    ...
    const settingsDir = path.join(tempWorkspaceDir, '.blackboxcli');
...
describe('installExtension', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(os.homedir).mockReturnValue(tempHomeDir);
    userExtensionsDir = path.join(tempHomeDir, '.blackboxcli', 'extensions');
  ...
  it('should throw an error and cleanup if blackbox-extension.json is missing', async () => {
    ...
    await expect(
      ...
      `Invalid extension at ${sourceExtDir}. Please make sure it has a valid blackbox-extension.json file.`,
    );
  ...
  it('should install an extension from a git URL', async () => {
    const gitUrl = 'https://github.com/google/blackbox-extensions.git';
    const extensionName = 'blackbox-extensions';
...
describe('performWorkspaceExtensionMigration', () => {
  ...
  it('should install the extensions in the user directory', async () => {
    ...
    const userExtensionsDir = path.join(tempHomeDir, '.blackboxcli', 'extensions');
...
function createExtension(
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'BLACKBOX.md'), 'context');
  }
