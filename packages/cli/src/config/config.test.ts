...
import { ShellTool, EditTool, WriteFileTool } from '@blackbox_ai/blackbox-cli-core';
import { loadCliConfig, parseArguments, type CliArgs } from './config.js';
import type { Settings } from './settings.js';
import type { Extension } from './extension.js';
import * as ServerConfig from '@blackbox_ai/blackbox-cli-core';
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/BLACKBOX.md'],
      },
      {
        path: '/path/to/ext2',
        config: {
    ...
    expect(config.getSystemPromptMappings()).toEqual([
      {
        ...
        modelNames: ['blackbox3-coder-plus'],
        template:
          'SYSTEM_TEMPLATE:{"name":"blackbox3_coder","params":{"is_git_repository":{RUNTIME_VARS_IS_GIT_REPO},"sandbox":"{RUNTIME_VARS_SANDBOX}"}}',
      },
    ]);
  });
...
describe('loadCliConfig model selection', () => {
  it('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('blackbox3-coder-plus');
  });
  ...
  it('always prefers model from argvs', async () => {
    process.argv = ['node', 'script.js', '--model', 'blackbox3-coder-plus'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {
        model: {
          name: 'blackbox3-coder-flash',
        },
      },
      [],
      'test-session',
