...
export function getSystemSettingsPath(): string {
  if (process.env['BLACKBOX_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['BLACKBOX_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/BlackboxCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\blackbox-cli\\settings.json';
  } else {
    return '/etc/blackbox-cli/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['BLACKBOX_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['BLACKBOX_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
